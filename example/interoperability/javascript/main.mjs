...
const languages = ['cpp', 'java', 'js', 'go', 'csharp'];
const variants = 3;
...
function exchange(directory, mode) {
  ...
  for (let variant = 0; variant < variants; ++variant) {
    ...
    for (const [name, protocol] of Object.entries(Protocol)) {
      ...
      if (mode === 'emit') writeFileSync(join(directory, `js_${name}_${variant}.bin`), bytes);
        ...
        assert.deepEqual(actual.encode(Protocol.BINARY_NONE), expected.encode(Protocol.BINARY_NONE), `${language} -> js ${name}/${variant}`);
...
else if (directory && ['emit', 'verify'].includes(mode)) { exchange(directory, mode); console.log(`js ${mode} passed`); }
