...
// Exercise the gnu profile's renamed owning API, mutable view, and unchanged
// wire names.
int
main ()
{
  ...
  using mutable_record
      = style_demo::account_record<storage_mode::mutable_view>;
  ...
  auto editor = mutable_record::map (
  ...
  decoder.finish ();
  if (decoded.account_id != 43
      || decoded.current_state != style_demo::account_state::active)
    {
      return 1;
    }
