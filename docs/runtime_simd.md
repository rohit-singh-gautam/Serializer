...
element-wise encoding; nested numeric vectors still reach the bulk helper.
...
operation benefits from vector instructions. Binary input retains its existing
element-wise decoder and resource accounting.
...
SIMD backends. Bulk numeric-array writes and direct JSON output remain enabled;
